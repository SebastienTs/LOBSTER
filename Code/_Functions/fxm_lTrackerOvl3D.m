...
function [] = fxm_lTrackerOvl3D(InputFolder,OutputFolder,params)

    % Track objects in 3D binary mask time-lapse by locally optimizing overlap-distance metric.
    %
    % Sample journal: <a href="matlab:JENI('TestMovie3D_TrackOvl3D.jlm');">TestMovie3D_TrackOvl3D.jlm</a>
    %
    % Input: 3D+T binary mask
    % Output: 3D+T label mask
    %
    % Parameters:
    ...
    se3d = ones(3,3,3);         % Structure element used for overlap map dilations (outside objects and inside particles)
    ...
    for kf = 1:num_images
        ...
        A = padarray(A,[0 0 1]);
        A = imclearborder(A,8);   % Remove objects touching edges
        ...
        if kf>1
            ...
            if OvlItExtend>0
                ObjLblExt = ObjLbl;   
                for i = 1:OvlItExtend
                    ObjLblExt2 = imdilate(ObjLblExt,se3d);
            ...
            OvlMapOld = OvlMap;
            while test>0
                OvlMapBuf = imdilate(OvlMap,se3d).*(PartLbl > 0);
        ...
        if (kf == 1)
            ...
            if Graph==1
                ...
                imagesc(max(ObjLbl,[],3),[0 ScaleImg]);
            ...
            if ((Graph == 1) && (FigParticles > -1))
                figure(FigParticles);
                imagesc(max(PartLbl,[],3),[0 ScaleImg]);
            ...
            if ((Graph == 1) && (FigOverlap > -1))
                figure(FigOverlap);
                imagesc(max(OvlMap,[],3),[0 ScaleImg]);
            ...
            for i = 1:Npart
                ...
                CMZVector = zeros(1,length(UniqueIndx));
                for k =1:length(UniqueIndx)
                    Msk = (Indx == UniqueIndx(k));
                    AreaVector(k) = sum(Msk);
                    [Y X Z] = ind2sub(size(A),Particles{i}(Msk));
                    CMXVector(k) = mean(X);
                    CMYVector(k) = mean(Y);
                    CMZVector(k) = mean(Z);
                end
            ...
            for i = 1:NCluster
                ...
                for j = 1:size(PartStates,1)   
                    ...
                    for k = 1:NPartSet
                        ...
                        ObjSubSetCMZ = PartObjCMZ{PartSet(k)};
                        ...
                        if (CurrentState(k) == TotPartStates(k)) % Share particle between different objects
                            ...
                            if (NObjSubSet == 1) % Single object inside particle
                                ...
                                NewCMZObj(ObjSubSetIndx(1)) = CMPartZ(PartSet(k));
                                ...
                                for l = 1:NObjSubSet % General case: each object takes its own share
                                    ...
                                    NewCMZObj(ObjSubSetIndx(l)) = (NewCMZObj(ObjSubSetIndx(l))*NewAreaObj(ObjSubSetIndx(l))+ObjSubSetCMZ(l)*ObjSubSetArea(l))/(NewAreaObj(ObjSubSetIndx(l))+ObjSubSetArea(l));
                            ...
                            if ( ( CurrentState(k) == (TotPartStates(k)-1) )&&( UnassignState == 1 ) )
                                ...
                                NewCMZObj(ObjSubSetIndx(CurrentState(k))) = CMPartZ(PartSet(k));
                    ...
                    DiffCM = sum(((CMObjX(ObjSet)-NewCMXObj(ObjSet)).*(NewAreaObj(ObjSet)>0)).^2+((CMObjY(ObjSet)-NewCMYObj(ObjSet)).*(NewAreaObj(ObjSet)>0)).^2+((CMObjZ(ObjSet)-NewCMZObj(ObjSet)).*(NewAreaObj(ObjSet)>0)).^2);
                    ...
                    if Diff<BestDiff
                        ...
                        BestObjCMZ(ObjSet) = NewCMZObj(ObjSet);
                ...
                for i=1:CC.NumObjects 
                    ...
                    if any(Disappeared)
                        for j=1:length(DisappearedIndx)
                            Dist = sqrt((LastCMObjX(DisappearedIndx(j))-NewCMObjX(i))^2+(LastCMObjY(DisappearedIndx(j))-NewCMObjY(i))^2+(LastCMObjZ(DisappearedIndx(j))-NewCMObjZ(i))^2);
                    ...
                    if BestMultObj(i)>1 % Various particles assigned to the same object  
                        ...
                        if (CC.NumObjects > 1) % Particles are not closely touching: candidate division
                            ...
                            for j=1:CC.NumObjects
                                [Y X Z] = ind2sub(size(A),CC.PixelIdxList{j});
                                Dist = sqrt((mean(X)-LastCMObjX(i))^2+(mean(Y)-LastCMObjY(i))^2+(mean(Z)-LastCMObjZ(i))^2);
                                if(Dist<MinDist)
                                    MinDistIndx = j;
                                    MinDist = Dist;
                                end
                        ...
                        %% Candidates are closely touching: candidate merging        
                             ...
                             [Y X Z] = ind2sub(size(A),CC.PixelIdxList{1});
                             CMObjX(i) = mean(X);
                             CMObjY(i) = mean(Y);
                             CMObjZ(i) = mean(Z);
                        end
        ...
        if Export==1
            imwrite(uint16(ObjLbl(1:end,1:end,2)), strcat(OutputFolder,Files(kf).name), 'Compression','deflate');
            for i = 3:size(ObjLbl,3)-1
                imwrite(uint16(ObjLbl(1:end,1:end,i)), strcat(OutputFolder,Files(kf).name), 'WriteMode', 'append', 'Compression','deflate');
            end
        end
